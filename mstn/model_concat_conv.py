...
class Conv2Plus1DKeepTimeScale(nn.Sequential):
    ...
    def __init__(self,
                 ...
                 stride=1,
                 padding=1):
        super(Conv2Plus1DKeepTimeScale, self).__init__(
            nn.Conv3d(in_planes, midplanes, kernel_size=(1, 3, 3),
                      stride=(1, stride, stride), padding=(0, padding, padding),
                      bias=False),
            nn.BatchNorm3d(midplanes),
...
class MSLT(nn.Module):
    ...
    def __init__(self, pretrained=False, progress=True, block=BasicBlock,
                 conv_makers=[Conv2Plus1DKeepTimeScale] * 2 + [Conv2Plus1DSpatioalEnhanced] * 2,
                 layers=[2, 2, 2, 2],
        ...
        self.fusion1 = nn.Conv3d(256*2, out_channels=256, kernel_size=1, stride=1, padding=0)
        self.fusion2 = nn.Conv3d(512*2, out_channels=512, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, stream):
        ...
        x = self.layer3(x)
        b, c, t, w, h = x.shape
        pooling_layer3_feature = self.pooling(layer3_feature)
        x = self.fusion1(torch.cat([x, pooling_layer3_feature], dim=1))

        x = self.layer4(x)
        b, c, t, w, h = x.shape
        pooling_layer4_feature = self.pooling(layer4_feature)
        x = self.fusion2(torch.cat([x, pooling_layer4_feature], dim=1))
...
class LSMT(nn.Module):
    ...
    def __init__(self, pretrained=True, progress=True, block=BasicBlock,
                 conv_makers=[Conv2Plus1DKeepTimeScale] * 2 + [Conv2Plus1DTemporalEnhanced] * 2,
                 layers=[2, 2, 2, 2],
...
def model_complexity_metric():
    ...
    net = MSTN(n_classes=400,alpha=2)
