...
class MSLT(nn.Module):
    ...
    def __init__(self, pretrained=False, progress=True, block=BasicBlock,
                 conv_makers=[Conv2Plus1DKeepTimeScale] * 3 + [Conv2Plus1DSpatioalEnhanced] * 1,
                 layers=[3, 4, 6, 3],
    ...
    def forward(self, x, stream):
        ...
        x = x + pooling_layer3_feature

        x = self.layer4(x)
        b, c, t, w, h = x.shape
        pooling_layer4_feature = self.pooling(layer4_feature)
        x = x + pooling_layer4_feature
...
class LSMT(nn.Module):
    ...
    def __init__(self, pretrained=True, progress=True, block=BasicBlock,
                 conv_makers=[Conv2Plus1DKeepTimeScale] * 3 + [Conv2Plus1DTemporalEnhanced] * 1,
                 layers=[3, 4, 6, 3],
