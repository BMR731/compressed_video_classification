...
class DepthwiseConv1DLayer(nn.Module):
    ...
    def __init__(self, n_channels, kernel_size, padding, stride):
        ...
        self.depthwise_conv1d = nn.Conv1d(n_channels, n_channels, kernel_size, stride=stride,padding=padding)
...
class Timeception(nn.Module):
    ...
    def __init__(self, n_channels):
        ...
        self.bottleneck = nn.Conv3d(n_channels, channels, kernel_size=1)

        self.branch1 = DepthwiseConv1DLayer(channels, kernel_size=1, stride=1, padding=0)  # 局部一点
        self.branch2 = DepthwiseConv1DLayer(channels, kernel_size=3, stride=1, padding=1)
        self.branch3 = DepthwiseConv1DLayer(channels, kernel_size=5, stride=1, padding=2)  # 全局一点
...
class MSLT(nn.Module):
    ...
    def __init__(self, alpha, pretrained=False, progress=True, block=BasicBlock,
        ...
        self.fusion1 = nn.Conv3d(128 // self.alpha, 128 // self.alpha, kernel_size=(3, 1, 1),
                                 stride=(self.alpha, 1, 1), padding=(1, 0, 0))
        self.fusion2 = nn.Conv3d(256 // self.alpha, 256 // self.alpha, kernel_size=(3, 1, 1),
                                 stride=(self.alpha, 1, 1), padding=(1, 0, 0))
        self.fusion3 = nn.Conv3d(512 // self.alpha, 512 // self.alpha, kernel_size=(3, 1, 1),
                                 stride=(self.alpha, 1, 1), padding=(1, 0, 0))
...
class LSMT(nn.Module):
    ...
    def __init__(self, alpha, pretrained=False, progress=True, block=BasicBlock,
                 ...
                 layers=[2, 2, 2, 2], input_channels=2,
                 zero_init_residual=False):
        """Generic resnet video generator.
...
class MSTN(nn.Module):  # multi-scale spational and temporal network
    def __init__(self, num_classes, alpha, dropout=0.5):
        ...
        self.fc = nn.Linear(512 + 512 // alpha, num_classes)
