...
class MSLT(nn.Module):
    ...
    def __init__(self, pretrained=False, progress=True, block=Bottleneck,
                 conv_makers=[Conv2Plus1DKeepTimeScaleNoDownsample] * 2 + [Conv2Plus1DKeepTimeScaleDownsample] * 2,
                 layers=[3, 4, 6, 3],
                 stem=SpatialStem, input_channels=3, num_classes=400, alpha=2,
        ...
        self.conv3 = nn.Conv3d(1024 + 1024 // self.alpha, 1024, kernel_size=1, stride=1, padding=0)
        self.fusion4 = nn.Conv3d(2048 // self.alpha, 2048 // self.alpha, kernel_size=(3, 1, 1),
                                 stride=(self.alpha, 1, 1), padding=(1, 0, 0))
        self.conv4 = nn.Conv3d(2048 + 2048 // self.alpha, 2048, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, stream):
        ...
        layer3_feature, layer4_feature = stream
        x = self.stem(x)
        ...
        x = self.layer3(x)
        b, c, t, w, h = x.shape
        y = torch.cat([self.fusion3(layer3_feature), x], dim=1)
        x = self.conv3(y)

        x = self.layer4(x)
        b, c, t, w, h = x.shape
        y = torch.cat([self.fusion4(layer4_feature), x], dim=1)
        x = self.conv4(y)
...
class LSMT(nn.Module):
    ...
    def __init__(self, alpha=2, pretrained=False, progress=True, block=Bottleneck,
                 conv_makers=[Conv2Plus1DKeepTimeScaleNoDownsample] * 2 + [Conv2Plus1DTemporalEnhanced] * 2,
                 layers=[3, 4, 6, 3], input_channels=2, num_classes=400,
                 zero_init_residual=False):
        """Generic resnet video generator.
    ...
    def forward(self, x):
        ...
        return layer1_feature, layer2_feature, layer3_feature, layer4_feature, x
...
class MSTN(nn.Module):  # multi-scale spational and temporal network
    ...
    def forward(self, x):
        # x: (iframe, mv)
        iframe, mv = x
        _, _, layer3_feature, layer4_feature, flatten1 = self.lsmt(mv)
        flatten2 = self.mslt(iframe, (layer3_feature, layer4_feature))
        score = self.fc(torch.cat([flatten1, flatten2], dim=1))
        return score
...
def model_complexity_metric():
    ...
    def input_constructor(input_res):
        ...
        mv1 = torch.randn(size=(1, 2, 2, 224, 224))
        return {'x': [im1, mv1]}

    net = MSTN(num_classes=400, alpha=2)
...
if __name__ == "__main__":
    num_classes = 101
    # # mv sample 2x iframe
    iframe = torch.rand(4, 3, 4, 224, 224)
    mv = torch.rand(4, 2, 8, 224, 224)
    model = MSTN(num_classes=400, alpha=2)
