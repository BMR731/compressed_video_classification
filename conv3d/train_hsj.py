...
from conv3d.model import IframeNet
...
SAVE_FREQ = 5
PRINT_FREQ = 20
ACCUMU_STEPS = 5  # use gradient accumlation to use least memory and more runtime
loss_min = 1
CONTINUE_FROM_LAST = True
LAST_SAVE_PATH = r'bt_80_seg_5_wd_0.000100_conv3d-train-joint__best.pth.tar'
FINETUNE = False
...
def main():
    ...
    description = 'bt_%d_seg_%d_wd_%f_%s' % (
        args.batch_size * ACCUMU_STEPS, args.num_segments, args.weight_decay, "conv3d-train-joint")
    ...
    for key, value in params_dict.items():
        ...
        if 'fc' in key:
            params += [{'params': [value], 'lr': args.lr*2, 'decay_mult': decay_mult}]
        elif 'conv1x1' in key:
...
def train(train_loader, model, criterions, optimizer, epoch):
    ...
    for i, (input_pairs, label) in enumerate(train_loader):
        ...
        iscores, mscores = model(input_pairs)
...
loss_min = 1
CONTINUE_FROM_LAST = True
LAST_SAVE_PATH = r'bt_80_seg_3_wd_0.000100_mv_res34_iframe_res152_mgc_train_joint__best.pth.tar'
FINETUNE = False
...
def main():
    ...
    global description
    description = 'bt_%d_seg_%d_wd_%f_%s' % (
        args.batch_size * ACCUMU_STEPS, args.num_segments, args.weight_decay, "debug")
    log_name = './log/%s' % description
    WRITER = SummaryWriter(log_name)
    print('Training arguments:')
    for k, v in vars(args).items():
    ...
    for key, value in params_dict.items():
        decay_mult = 0.0 if 'bias' in key else 1.0
        if 'module.fc' in key:
            params += [{'params': [value], 'lr': args.lr * 30, 'decay_mult': decay_mult}]
        elif 'conv1x1' in key:
            params += [{'params': [value], 'lr': args.lr * 30, 'decay_mult': decay_mult}]
        elif 'module.mvnet' in key:
            params += [{'params': [value], 'lr': args.lr * 100, 'decay_mult': decay_mult}]
        else:
            params += [{'params': [value], 'lr': args.lr * 3, 'decay_mult': decay_mult}]
