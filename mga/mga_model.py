from mga.ResNet import ResNet34, ResNet152
import torch
...
class MvNet(ResNet34):
    def __init__(self,num_segments):
        super(MvNet, self).__init__(input_channels=2, num_classes=1000, pretrained=True)
        self.num_segments = num_segments
        self.dropout = nn.Dropout(DROPOUT)
        self.fc_layer1 = nn.Linear(1000,512)
        self.cls_layer = nn.Linear(512,101)


    def forward_hh(self, inputs):
        _,x = inputs
        x = x.view((-1, ) + x.size()[-3:])
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        return output,low_level_feature,layer4_feature
    ...
    def forward(self, x):
        ...
        output = x.view((-1, self.num_segments) + x.size()[1:])
        return conv1_feature, low_level_feature, layer2_feature, layer3_feature, layer4_feature, x

class IframeNet(ResNet152):
    def __init__(self,num_segments):
        super(IframeNet, self).__init__(input_channels=3, num_classes=1000, pretrained=True)
        self.num_segments = num_segments
    ...
    def forward(self, inputs):
        img,mv = inputs
        mv_conv1_feature, mv_layer1_feature, mv_layer2_feature, mv_layer3_feature, mv_layer4_feature, _ = self.mvnet(mv)
